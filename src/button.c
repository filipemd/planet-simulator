...
bool Button(Rectangle rect, const char* text, bool centered) {
    ...
    if (clicked) {
        DrawRectangle(button_rect.x, button_rect.y, button_rect.width+2, button_rect.height+2, WHITE);
    } else {
        DrawRectangle(button_rect.x-2, button_rect.y-2, button_rect.width+2, button_rect.height+2, WHITE);
    }
